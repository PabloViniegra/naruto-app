...
import type { ClanWithMembers, ClanSize } from "../types";
import { getClanSize } from "../types";

interface ClanBentoCardProps {
...
export function ClanBentoCard({ clan, index, className }: ClanBentoCardProps) {
  const size = getClanSize(clan.characters.length);
      ...
      >
        ...
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-1 rounded-full bg-background/80 px-2 py-1 text-xs font-medium backdrop-blur-sm">
            ...
            <span>{clan.characters.length}</span>
          </div>
        </div>
        ...
        <div className={cn("mt-4", size === "large" && "mt-auto")}>
          {clan.members.length > 0 ? (
            <AvatarStack
              ...
              totalCount={clan.characters.length}
              size={size}
            />
          ) : (
        ...
        {size === "large" && (
          <div className="mt-4 text-sm text-muted-foreground">
            <p>
              One of the major clans with {clan.characters.length} known members
            </p>
          </div>
        )}
      </div>
